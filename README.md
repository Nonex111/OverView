...
All the technical stuff are done by the member with the role of technical expert. So the functionality about scraping comments and sending the result plots to the frontend is incomplete due to lack of time. Its implementation is similar to the process of interaction between frontend and backend when calling GPT API for analyses.
And the part of calling GPT API is commented for demo. You can uncomment it if you want to reproduce that functionality.
...
### Steps to test functionalities or continue on this project
